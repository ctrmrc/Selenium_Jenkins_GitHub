...
if count_test_start_txt > count_pass_txt:
    os.system('curl -s -X POST https://api.telegram.org/bot5698947661:AAFmFW1PEUa7STMd6yXq9x91tJ2oyavqKAE/sendMessage -d chat_id=-746480341 -d text="%JOB_NAME% Build %BUILD_NUMBER% finish with FAILURE"')
elif count_test_start_txt == count_pass_txt:
    os.system('curl -s -X POST https://api.telegram.org/bot5698947661:AAFmFW1PEUa7STMd6yXq9x91tJ2oyavqKAE/sendMessage -d chat_id=-746480341 -d text="%JOB_NAME% Build %BUILD_NUMBER% finish with SUCCESS"')
else:
    print('ERROR')
